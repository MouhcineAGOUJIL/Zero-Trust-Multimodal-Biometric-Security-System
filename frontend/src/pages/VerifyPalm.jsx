...
import { Hand, CheckCircle, XCircle, Camera, Loader, Upload } from 'lucide-react';
...
function VerifyPalm() {
    ...
    const [uploadMode, setUploadMode] = useState(true); // Default to Upload for Palm
    ...
    const verify = async () => {
        ...
        if (!imgSrc && !file) { setStatus('error'); setMessage('Please capture or upload palm image'); return; }

        setStatus('loading'); setMessage('Processing Palm...');
        ...
        try {
            ...
            if (file) {
                formData.append('file_palm', file);
            } else {
                const blob = await (await fetch(imgSrc)).blob();
                formData.append('file_palm', blob, 'palm.jpg');
            }

            const res = await axios.post('http://127.0.0.1:8000/auth/verify/palm', formData);
        ...
        <div className="page-container">
            <div className="card">
                <div className="header">
                    <Hand size={24} color="#ec4899" />
                    <h1>Palm Verification</h1>
                </div>
                ...
                <div className="form-stack">
                    ...
                    <div className="form-group">
                        ...
                        {uploadMode ? (
                            <label className={`file-upload ${file ? 'border-pink-500' : ''}`}>
                                <Upload size={20} className="icon-pink" />
                                <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis' }}>{file ? file.name : "Upload Palm Image"}</span>
                    ...
                    >
                        {status === 'loading' ? <Loader className="spin" /> : 'Verify Palm'}
                    </button>
